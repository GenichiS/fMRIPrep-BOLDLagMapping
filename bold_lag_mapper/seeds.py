...
input is in that space (without --native-space), or when the input is fMRIPrep output in the subject's T1w
space processed with --native-space (the mask is then moved to T1w space with fMRIPrep's
MNI152NLin2009cAsym-to-T1w transform). For any other input - another template, HCP data, a file without a
space- entity - the caller must choose: a seed mask in the input's space, or the global mean signal.
"""
import logging
...
def resolve_seed_roi(value, bold_file, native_space, auto_seed_from_freesurfer=False):
    ...
    if str(value).strip().lower() == "builtin":
        ...
        parsed = BIDSPathParser(bold_file)
        space = parsed.entities.get("space")
        mni_input = space == BUILTIN_SEED_SPACE and not native_space
        fmriprep_t1w_native = native_space and space == "T1w" and parsed.pipeline_type == "fmriprep"
        if mni_input or fmriprep_t1w_native:
            logger.info(f"Seed: bundled deep white-matter mask ({BUILTIN_SEED_SPACE}).")
            return str(BUILTIN_SEED_PATH)
        raise ValueError(
            f"The bundled seed is defined in {BUILTIN_SEED_SPACE}; it can be used for {BUILTIN_SEED_SPACE} input "
            "without --native-space, or for fMRIPrep T1w-space input with --native-space. The input BOLD "
            f"({Path(bold_file).name}) is in space '{space or 'unknown'}' (pipeline: {parsed.pipeline_type}, "
            f"--native-space {'on' if native_space else 'off'}). Give --seed-roi-file PATH (a mask in "
            f"{BUILTIN_SEED_SPACE} with --native-space, else in the input's space) or --seed-roi-file global.")
    path = Path(value)
    if not path.exists():
